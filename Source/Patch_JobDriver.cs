...
namespace SafeTemperature
{
    ...
    public static class Patch_JobDriver_DriverTick
    {
        ...
        {
            if (!___pawn.Downed && ___pawn.jobs.curJob.def != DefDatabase<JobDef>.GetNamed("GotoSafeTemperature") && ___pawn.jobs.curJob.def != DefDatabase<JobDef>.GetNamed("Wait_SafeTemperature"))
            {
